...
describe 'webSocket', ->
  ...
  describe 'initialization', ->
    ...
    xit 'should construct URL that matches Really URL scheme when domain is passed', ->
    ...
    xit 'should initialize socket', ->
      ws = new WebSocketTransport('wss://a6bcc.api.really.io', 'ibj88w5aye', options)
      expect(ws.socket).toBeNull()

    xit 'should initialize callbacks buffer', ->
      ws = new WebSocketTransport('wss://a6bcc.api.really.io', 'ibj88w5aye', options)
      expect(ws.callbacksBuffer).toEqual new CallbacksBuffer()

    xit 'should initialize messages buffer', ->
      ws = new WebSocketTransport('wss://a6bcc.api.really.io', 'ibj88w5aye', options)
      expect(ws._messagesBuffer).toEqual []

    xit 'should initialize push handler', ->
      ws = new WebSocketTransport('wss://a6bcc.api.really.io', 'ibj88w5aye', options)
      expect(ws.pushHandler).toEqual PushHandler

    xit 'should set initialized to false', ->
      ws = new WebSocketTransport('wss://a6bcc.api.really.io', 'ibj88w5aye', options)
      expect(ws.initialized).toBeFalsy()

    xit 'should put appropriate default values for options if not supplied', ->
      defaultValues =
        reconnectionMaxTimeout: 30e3
      ...
      # expect(ws1.options.reconnectionMaxTimeout).toEqual options1.reconnectionMaxTimeout
      # expect(ws1.options.heartbeatTimeout).toEqual defaultValues.heartbeatTimeout
      # expect(ws1.options.heartbeatInterval).toEqual defaultValues.heartbeatInterval
      # expect(ws1.options.reconnect).toEqual defaultValues.reconnect
      # expect(ws1.options.onDisconnect).toEqual defaultValues.onDisconnect
      ...
      # expect(ws2.options.reconnectionMaxTimeout).toEqual options2.reconnectionMaxTimeout
      # expect(ws2.options.heartbeatTimeout).toEqual options2.heartbeatTimeout
      # expect(ws2.options.heartbeatInterval).toEqual defaultValues.heartbeatInterval
      # expect(ws2.options.reconnect).toEqual defaultValues.reconnect
      # expect(ws2.options.onDisconnect).toEqual defaultValues.onDisconnect
    ...
    xit 'should fire events per each instance', ->
      ...
      ws1.on 'test', ->
        console.log "im in test callback emitter1"
        emitter1 = true
      ...
      ws2.on 'test', ->
        console.log "im in test callback emitter2"
        emitter2 = true
  ...
  describe 'connect', ->
    ...
    xit 'should initialize socket first time it try to connect', ->
    ...
    xit 'should use the same socket object everytime it tries to connect when channel is CLOSED', (done) ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN,'ibj88w5aye', options)
      ws.connect()
      socket1 = ws.socket
      expect(socket1).toBeDefined()
    ...
    xit 'should initialize socket when try to connect with different domain and token', ->
    ...
    describe 'connection open', ->
      xit 'should initialize reconnectiong attempts number to zero', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
      ...
      xit 'should reslove a promise when connection open', (done) ->
         ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
         promise = ws.connect()
         ws.socket.addEventListener 'open', () ->
          promise.done( (data) ->
      ...
      xit 'should send first message when connection open', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        spyOn(ws, 'send')
      ...
      xit 'should fire "opened" event when connection open', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        spyOn(ws, 'emit')
        ws.socket.addEventListener 'open', () ->
    ...
    describe 'connection error', ->
      xit 'should fire "error" event', (done) ->
    ...
    describe 'connection message', ->
      xit 'should fire "message" event', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        spyOn(ws, 'emit')
      ...
      xit 'should handle buffered message callbacks if it has a tag', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        ws.socket.addEventListener 'open', () ->
    ...
    describe 'connection close', ->
      xit 'should fire "reconnecting" event when reconnect options is true', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        spyOn(ws, 'emit')
        ws.socket.addEventListener 'close', () ->
      ...
      xit 'should reconnect when reconnect option is true', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        spyOn(ws, 'reconnect')
        ws.socket.addEventListener 'close', () ->
      ...
      xit 'should fire "close" event when reconnect options is false', (done) ->
        newOptions =
          reconnect: false 
      ...
      xit 'should disconnect when reconnect option is false', (done) ->
        newOptions =
          reconnect: false 
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', newOptions)
        ws.connect()
  ...
  describe 'send', ->
    describe 'channel connected', ->
      ...
      xit 'should send message with tag', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
      ...
      xit 'should buffer message callbacks', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        message = protocol.createMessage('/users')
        {kind} = message
      ...
      xit 'should send string message with tag', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        message = protocol.createMessage('/users')
      ...
      xit 'should return promise', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        message = protocol.createMessage('/users')
        ws.socket.addEventListener 'open', () ->
      ...
      xit 'should reslove the promise when success callback occurs', ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        message = protocol.createMessage('/users')
      ...
      xit 'should reject the promise when error callback occurs', (done) ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        ws.connect()
        message = 
          kind: 'give-me-error'
        ...
        ws.socket.addEventListener 'open', () ->
          promise = ws.send(message, {success, error, complete})
          promise.catch (e) ->
            expect(errorMessage).toEqual e
            ws = null
            done()

    describe 'channel not connected', ->

      xit 'should buffer messages if strategy chosen is "buffer"', ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        #ws.connect()
        message = protocol.createMessage('/users')
      ...
      xit 'should call custom message if strategy chosen is "custom"', (done)->
      ...
      xit 'should fail messages if strategy chosen is "fail" by rejecting the promise', ->
        newOptions =
          onDisconnect: 'fail'
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', newOptions)
      ...
      xit 'should return a promise when channel is not connected with any strategy', ->
        ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
        message = protocol.createMessage('/users')
        promise = ws.send(message, {})
        expect(typeof promise.then is 'function').toBeTruthy()
  ...
  describe 'reconnect', ->
    xit 'should increase number of attempts with one', (done) ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5ake', options)
      ws.connect()
      ws.socket.addEventListener 'open', () ->
        ws.socket.close()
    ...
    xit 'should reconnect when time out', (done) ->
  ...
  describe 'disconnect', ->
    ...
    xit 'should close the websocket transport', (done) ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5aye', options)
      ws.connect()
    ...
    xit 'should set the initialized flag to false', (done) ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5aye', options)
      ws.connect()
    ...
    xit 'should set the socket instance to null', (done) ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5aye', options)
      ws.connect()
  ...
  describe 'isConnecting', ->
    ...
    xit 'should return false if socket is not initialized', ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5aye', options)
      ws.connect()
    ...
    xit 'should return true if socket is connecting',  ->
      ws = new WebSocketTransport(CONFIG.REALLY_DOMAIN, 'ibj88w5aye', options)
      ws.connect()
